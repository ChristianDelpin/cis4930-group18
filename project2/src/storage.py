...
def save_the_records(records):
    ...
    if os.path.exists(OUTPUT_PATH):
        ...
        print(f"[INFO] Appended {len(records)} rows to the existing csv, congrats.") # TODO: Change to use logging

    else:
        df.to_csv(OUTPUT_PATH, index=False)
        print(f"[INFO] Creating new csv with {len(records)} rows, good job.") # TODO: Change to use logging
